...
function checkSubscription() {
    ...
    if (!currentShopID) {
        ...
        if(landingPage) { landingPage.classList.remove('hidden'); landingPage.style.display = 'flex'; }
        return;
    }
...
function guestLogin() {
    ...
    let trialStart = new Date().getTime();
    ...
    localStorage.setItem('smartpos_wanumber', phone.trim());
    localStorage.setItem('smartpos_shopname', "Guest Shop");

    currentShopID = shopID; shopName = "Guest Shop"; waNumber = phone.trim();

    db.collection('shops').doc(shopID).set({
        shopName: "Guest Shop", waNumber: phone.trim(), isTrial: true, createdAt: new Date().toLocaleString()
    }, { merge: true }).then(() => {
        alert("✅ আপনার ১৫ দিনের ফ্রি ট্রায়াল শুরু হয়েছে!"); checkSubscription();
    }).catch(err => {
        alert("✅ ট্রায়াল শুরু হয়েছে! (Offline Mode)"); checkSubscription();
    });
}

function forceLogout() {
    localStorage.removeItem('smartpos_shop_id');
    currentShopID = null; checkSubscription();
}
...
function resetShopPassword(shopID) {
    if(confirm(`আপনি কি নিশ্চিত যে ${shopID} এর পাসওয়ার্ড রিসেট করে ডিফল্ট "1234" করতে চান?`)) {
        db.collection('shops').doc(shopID).set({ password: "1234" }, { merge: true }).then(() => { alert(`✅ ${shopID} এর পাসওয়ার্ড সফলভাবে রিসেট করে "1234" সেট করা হয়েছে!`); });
    }
}
...
function toggleShopStatus(shopID, makeActive) {
    ...
    if(confirm(`আপনি কি এই দোকানটি ${actionTxt} করতে চান?`)) { db.collection('licenses').doc(shopID).set({ isActive: makeActive }, { merge: true }).then(() => { alert(`সফলভাবে ${actionTxt} করা হয়েছে!`); loadMasterDatabase(); loadAdminStats(); }); }
}

function peekIntoShop(shopID) { currentShopID = shopID; let adminPanel = document.getElementById('super-admin-panel'); if(adminPanel) { adminPanel.classList.add('hidden'); adminPanel.style.display = 'none'; } loadCloudData(shopID); let posContainer = document.querySelector('.pos-container'); if(posContainer) posContainer.style.display = 'flex'; }
...
function cleanDeadShops() {
    ...
    db.collection('shops').get().then(snap => {
        snap.forEach(doc => {
            let shopID = doc.id;
            db.collection('licenses').doc(shopID).get().then(licDoc => { if(!licDoc.exists || licDoc.data().isActive === false) { db.collection('shops').doc(shopID).delete(); deadCount++; } });
        });
        ...
        if(doc.exists) {
            ...
            if(d.globalNotice && d.globalNotice !== "") {
                ...
                noticeStyleFix.innerHTML = `
                    ...
                    #sidebar-overlay { top: 32px !important; height: calc(100vh - 32px) !important; }
                `;
...
function downloadServerBackup() {
    ...
    let btn = document.getElementById('btn-backup-server'); if(btn) { btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Preparing...'; btn.disabled = true; }
    db.collection('shops').get().then(snap => {
...
function sendSupportTicket() { if(!currentShopID || currentShopID.startsWith("TRIAL-")) return alert("প্রো লাইসেন্স আপগ্রেড করুন!"); let issue = prompt("অ্যাডমিনকে আপনার সমস্যা বিস্তারিত লিখুন:"); if(!issue || issue.trim() === "") return; db.collection('support_tickets').add({ shopID: currentShopID, shopName: shopName, message: issue.trim(), reply: "", status: "Open", timestamp: new Date().getTime(), dateStr: new Date().toLocaleString() }).then(() => alert("✅ মেসেজ পাঠানো হয়েছে!")); }
function listenForSupportReplies() { if(!currentShopID || currentShopID.startsWith("TRIAL-")) return; db.collection('support_tickets').where('shopID', '==', currentShopID).where('status', '==', 'Closed').onSnapshot(snap => { snap.docChanges().forEach(change => { if (change.type === "added" || change.type === "modified") { let data = change.doc.data(); if(data.reply && data.reply !== "") { alert(`📩 অ্যাডমিন রিপ্লাই:\nআপনার প্রশ্ন: ${data.message}\nউত্তর: ${data.reply}`); db.collection('support_tickets').doc(change.doc.id).delete(); } } }); }); }
setTimeout(listenForSupportReplies, 5000);

function loadSupportTickets() { let tbody = document.getElementById('admin-ticket-list'); if(!tbody) return; db.collection('support_tickets').orderBy('timestamp', 'desc').onSnapshot(snap => { tbody.innerHTML = ''; if(snap.empty) return tbody.innerHTML = '<tr><td colspan="4" style="text-align:center;">কোনো নতুন সাপোর্ট রিকোয়েস্ট নেই 🎉</td></tr>'; snap.forEach(doc => { let d = doc.data(); let statusTag = d.status === "Open" ? `🟡 Pending` : `🟢 Replied`; let actionBtn = d.status === "Open" ? `<button class="btn btn-primary btn-small" onclick="replyToTicket('${doc.id}', '${d.shopID}', '${d.message}')">Reply</button>` : `<button class="btn btn-danger btn-small" onclick="deleteTicket('${doc.id}')">Delete</button>`; tbody.innerHTML += `<tr><td><b>${d.shopName}</b><br><small>${d.shopID}</small></td><td>${d.message}</td><td>${statusTag}</td><td>${actionBtn}</td></tr>`; }); }); }
...
function buyPlan(planName, price, days) {
    let myShopID = currentShopID || "Unknown ID"; let myShopName = shopName || "Unknown Shop"; let adminWaNum = "8801621244970"; 
...
// ================= DATA SYNC, NAVIGATION & DASHBOARD =================
    ...
    try {
        ...
        if (currentShopID && !currentShopID.startsWith("TRIAL-")) {
            db.collection('shops').doc(currentShopID).set({
                ...
                shopName: shopName || "My Shop", waNumber: waNumber || "", lastSync: new Date().toLocaleString()
            }, { merge: true });
        }
    } catch (e) { console.error(e); if (e.name === 'QuotaExceededError' || e.code === 22) { alert("সতর্কবার্তা: ব্রাউজারের মেমোরি ফুল হয়ে গেছে! দয়া করে কিছু বড় সাইজের ছবি ডিলিট করুন।"); } }
}
...
function loadCloudData(shopId) {
    db.collection('shops').doc(shopId).get().then((doc) => {
        if (doc.exists) { 
            let data = doc.data(); 
            customersDB = data.customers || []; productsDB = data.products || []; salesHistoryDB = data.sales || []; dueCollectionHistoryDB = data.due_collections || []; expensesDB = data.expenses || []; 
            shopName = data.shopName || "My Shop"; waNumber = data.waNumber || ""; 
            
            localStorage.setItem('smartpos_shopname', shopName); localStorage.setItem('smartpos_wanumber', waNumber);
            ...
            if(data.profilePic && data.profilePic.startsWith('data:image')) { localStorage.setItem('smartpos_user_pic', data.profilePic); } 
            else { localStorage.removeItem('smartpos_user_pic'); }
        } else {
            customersDB = []; productsDB = []; salesHistoryDB = []; dueCollectionHistoryDB = []; expensesDB = []; shopName = "My Shop"; waNumber = "";
            localStorage.setItem('smartpos_shopname', "My Shop"); localStorage.removeItem('smartpos_user_pic'); localStorage.removeItem('smartpos_user_name'); localStorage.removeItem('smartpos_user_email');
        }
        
        applyShopBranding(); applyGlobalBranding(); renderAllTables(); populateDropdowns(); switchPage('dashboard'); 
        if (document.getElementById('profile') && document.getElementById('profile').classList.contains('active')) { loadUserProfileData(); }
    });
}

function initializeApp() {
    checkSubscription(); applyGlobalBranding(); applyShopBranding(); 
...
function saveSettings() {
    ...
    if (waInput) { waNumber = waInput.value; localStorage.setItem('smartpos_wanumber', waNumber); }
    saveData(); alert("✅ Settings Saved Successfully!");
}
...
function switchPage(pageId) {
    ...
    document.querySelectorAll('.nav-menu li').forEach(li => {
        if(li.getAttribute('onclick') && li.getAttribute('onclick').includes(`'${pageId}'`)) li.classList.add('active');
    });
...
function checkNetworkStatus() { const badge = document.getElementById('network-badge'); if(!badge) return; if(navigator.onLine) { badge.innerHTML = '<i class="fa-solid fa-cloud-arrow-up"></i> Auto-Sync Active'; badge.style.background = '#10b981'; } else { badge.innerHTML = '<i class="fa-solid fa-wifi"></i> Offline Mode'; badge.style.background = '#ef4444'; } }
...
function printReport(areaId) { const printArea = document.getElementById('print-area'); const content = document.getElementById(areaId).innerHTML; const selectedSize = document.getElementById('setting-print-size') ? document.getElementById('setting-print-size').value : 'a4-portrait'; if(selectedSize === 'a4-landscape') { printArea.className = 'a4-print landscape'; } else { printArea.className = 'a4-print'; } printArea.innerHTML = content; setTimeout(() => { window.print(); }, 500); }

// ================= REAL EXACT PROFIT LOGIC =================
function switchDashboardView(viewMode) {
    ...
    salesHistoryDB.forEach(sale => { 
        ...
        // নতুন লজিকে sale.profit আছে, পুরানো ডাটার জন্য fallback হিসেবে 15% রাখা হলো
        ...
        if(sd.getMonth() === now.getMonth() && sd.getFullYear() === now.getFullYear()) { 
            mSell += sale.total; mProfit += profitAmt; 
        } 
        if(sd.toDateString() === now.toDateString()) { 
            tSell += sale.total; tProfit += profitAmt; countBills++; 
        } 
    });
    ...
    expensesDB.forEach(exp => { let ed = new Date(exp.date); if(ed.toDateString() === now.toDateString()) tCost += exp.amount; });
...
function initChart() { const ctx1 = document.getElementById('myChart'); if(!ctx1) return; let config = { type: 'line', data: { labels: ['6 Days Ago','5 Days Ago','4 Days Ago','3 Days Ago','2 Days Ago','Yesterday','Today'], datasets: [{ label: 'Sales Trend (৳)', data: [0,0,0,0,0,0,0], borderColor: '#3b82f6', backgroundColor: 'rgba(59, 130, 246, 0.2)', borderWidth: 2, fill: true, tension: 0.4 }] }, options: { responsive: true, maintainAspectRatio: false } }; if(typeof Chart !== 'undefined') salesChart = new Chart(ctx1, config); }
...
function renderAllTables() {
    ...
    if(prodBody) {
        ...
        productsDB.forEach((p) => {
            ...
            prodBody.innerHTML += `<tr><td style="display: flex; align-items:center; gap:10px;"><img src="${imgSrc}" style="width:40px; height:40px; border-radius:5px; object-fit:cover; border: 1px solid #e2e8f0;"><b>${p.name}</b></td><td>${stockStatus}</td><td>৳ ${p.buy}</td><td>৳ ${p.sell}</td><td class="no-print"><button class="btn btn-primary btn-small" onclick="editProduct('${p.name}')"><i class="fa-solid fa-pen"></i></button> <button class="btn btn-danger btn-small" onclick="deleteProduct('${p.name}')"><i class="fa-solid fa-trash"></i></button></td></tr>`;
        });
    ...
    if(cusBody) {
        ...
        customersDB.forEach((c) => {
            ...
            cusBody.innerHTML += `<tr><td onclick="viewCustomerLedger('${c.name}')" style="cursor:pointer; display: flex; align-items:center; gap:12px;"><img src="${imgSrc}" style="width:40px; height:40px; border-radius:50%; object-fit:cover; border: 1px solid #e2e8f0; background: #fff;"><div><b>${c.name}</b><br><small style="color:#64748b;">${c.phone}</small></div></td><td>${catTag}</td><td style="color:#ef4444; font-weight:bold;">৳ ${c.due.toFixed(2)}</td><td class="no-print"><button class="btn btn-success btn-small" onclick="sendDueWhatsApp('${c.phone}', '${c.name}', ${c.due})" title="Send Reminder"><i class="fa-brands fa-whatsapp"></i></button> <button class="btn btn-primary btn-small" onclick="editCustomer('${c.name}')"><i class="fa-solid fa-pencil"></i></button> <button class="btn btn-danger btn-small" onclick="deleteCustomer('${c.name}')"><i class="fa-solid fa-trash"></i></button></td></tr>`;
        });
    }
...
function saveNewCustomer() { const name = document.getElementById('newCusName').value.trim(); const phone = document.getElementById('newCusPhone').value.trim(); const due = parseFloat(document.getElementById('newCusDue').value) || 0; let imgData = document.getElementById('cus-img-preview').src; if(imgData.includes('via.placeholder.com')) imgData = ""; if (!name || !phone) return alert("নাম ও ফোন দিন!"); let editIdx = document.getElementById('editCusIndex').value; if(editIdx !== "-1") { let c = customersDB.find(cus => cus.name === editIdx); if(c) { c.name = name; c.phone = phone; c.due = due; c.img = imgData; } } else { if (!checkPlanLimit('customer')) return; if(customersDB.find(c => c.name.toLowerCase() === name.toLowerCase())) return alert("এই নামে অলরেডি কাস্টমার আছে!"); customersDB.push({ name, phone, category: "Normal", due, limit: 5000, img: imgData }); } saveData(); document.getElementById('addCustomerModal').classList.add('hidden'); renderAllTables(); populateDropdowns(); }
...
function saveNewProduct() { const name = document.getElementById('newProdName').value.trim(); const buy = parseFloat(document.getElementById('newBuyPrice').value) || 0; const sell = parseFloat(document.getElementById('newSellPrice').value) || 0; const stock = parseFloat(document.getElementById('newStock').value); let imgData = document.getElementById('img-preview').src; if(imgData.includes('via.placeholder.com')) imgData = ""; if (!name || isNaN(stock) || isNaN(sell)) return alert("সব তথ্য দিন!"); let editNameRef = document.getElementById('editProdIndex').value; if(editNameRef !== "-1") { let p = productsDB.find(prod => prod.name === editNameRef); if(p) { p.name = name; p.buy = buy; p.sell = sell; p.stock = stock; p.img = imgData; } } else { if (!checkPlanLimit('product')) return; productsDB.push({ name, stock, buy, sell, unit: 'Pcs', img: imgData, isTop: false }); } saveData(); document.getElementById('addProductModal').classList.add('hidden'); renderAllTables(); populateDropdowns(); }
...
function populateDropdowns() {
    ...
    const prodList = document.getElementById('prod-dropdown-list'); if(prodList) { prodList.innerHTML = ''; productsDB.forEach(p => { prodList.innerHTML += `<div class="item" onclick="selectProduct('${p.name}', ${p.sell}, ${p.buy})" style="padding:10px; border-bottom:1px solid #eee; cursor:pointer; display:flex; justify-content:space-between;"><span>${p.name} <small>(${p.stock})</small></span><b>৳${p.sell}</b></div>`; }); }
}
...
function finalizeSale(action) {
    ...
    // আসল প্রফিট হিসাব (আসল কেনার দাম দিয়ে)
    let totalBuyCost = 0;
    cartItems.forEach(i => { 
    ...
    // ডিসকাউন্ট বাদ দেওয়ার পর যেটা থাকে, সেটা থেকে কেনার দাম বাদ দিলেই আসল লাভ!
    let invoiceProfit = grandTotal - totalBuyCost;

    salesHistoryDB.unshift({ invoice: invoiceNo, date: date, rawDate: dateObj, customer: cusName, total: grandTotal, paid: paidAmt, due: currentDue, profit: invoiceProfit });
...
function renderSalesHistory(filterCus = "All") {
    ...
    salesHistoryDB.forEach(s => { if(filterCus === "All" || s.customer === filterCus) { tS += s.total; tP += s.paid; tD += s.due; tbody.innerHTML += `<tr><td>${s.invoice}</td><td>${s.date}</td><td>${s.customer}</td><td>৳ ${s.total.toFixed(2)}</td><td style="color:#10b981;">৳ ${s.paid.toFixed(2)}</td><td style="color:#ef4444;">৳ ${s.due.toFixed(2)}</td></tr>`; } });
...
function applyShopBranding() {
    ...
    let mobileName = document.getElementById('mobile-shop-name-display');
    if(mobileName) mobileName.innerText = shopNameText;
}
...
function loadUserProfileData() {
    ...
    if (savedPic && savedPic.startsWith('data:image')) { document.getElementById('display-profile-img').src = savedPic; } 
    else { document.getElementById('display-profile-img').src = "https://via.placeholder.com/100?text=User"; }
}

function saveUserProfile() {
    const uName = document.getElementById('profile-user-name').value; const sName = document.getElementById('profile-shop-name').value; const uPhone = document.getElementById('profile-user-phone').value; const uEmail = document.getElementById('profile-user-email').value; const uPic = document.getElementById('display-profile-img').src;
    localStorage.setItem('smartpos_user_name', uName); localStorage.setItem('smartpos_shopname', sName); localStorage.setItem('smartpos_user_phone', uPhone); localStorage.setItem('smartpos_user_email', uEmail);
    if(uPic && uPic.startsWith('data:image')) { localStorage.setItem('smartpos_user_pic', uPic); } else { localStorage.removeItem('smartpos_user_pic'); }
    shopName = sName; applyShopBranding(); 
    if (currentShopID && !currentShopID.startsWith("TRIAL-")) { db.collection('shops').doc(currentShopID).set({ ownerName: uName, shopName: sName, waNumber: uPhone, email: uEmail, profilePic: (uPic.startsWith('data:image') ? uPic : "") }, { merge: true }).then(() => { alert("✅ প্রোফাইল সফলভাবে আপডেট এবং ক্লাউডে সেভ হয়েছে!"); }); } 
    else { alert("✅ প্রোফাইল লোকালি সেভ হয়েছে! (ট্রায়াল মোড)"); }
}

function loginWithGoogle() {
    let loginBtn = document.getElementById('btn-google-login'); if(loginBtn) { loginBtn.disabled = true; loginBtn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> দয়া করে অপেক্ষা করুন...'; }
    const provider = new firebase.auth.GoogleAuthProvider(); provider.setCustomParameters({ prompt: 'select_account' }); 
    firebase.auth().signInWithPopup(provider).then((result) => {
        const user = result.user; const shopID = user.uid; 
        db.collection('licenses').doc(shopID).get().then(doc => {
            if(!doc.exists) { db.collection('licenses').doc(shopID).set({ shopID: shopID, validityDays: 15, isActive: true, email: user.email, createdAt: new Date().toLocaleString() }); } 
            else if (doc.data().isActive === false) { if(loginBtn) { loginBtn.disabled = false; loginBtn.innerHTML = '<i class="fa-brands fa-google"></i> Google দিয়ে লগইন করুন'; } alert("❌ আপনার অ্যাকাউন্টটি লক করা হয়েছে! দয়া করে অ্যাডমিনের সাথে যোগাযোগ করুন।"); return; }
            checkDeviceLimit(shopID, () => {
                currentShopID = shopID; localStorage.setItem('smartpos_shop_id', currentShopID); let validDays = doc.exists ? doc.data().validityDays : 15; let expiry = new Date().getTime() + (validDays * 24 * 60 * 60 * 1000); localStorage.setItem('smartpos_expiry', expiry);
                let landing = document.getElementById('saas-landing'); if(landing) { landing.style.display = 'none'; landing.classList.add('hidden'); } document.querySelector('.pos-container').style.display = 'flex'; loadCloudData(currentShopID);
                if(loginBtn) { loginBtn.disabled = false; loginBtn.innerHTML = '<i class="fa-brands fa-google"></i> Google দিয়ে লগইন করুন'; }
            });
        });
    }).catch((error) => { if(loginBtn) { loginBtn.disabled = false; loginBtn.innerHTML = '<i class="fa-brands fa-google"></i> Google দিয়ে লগইন করুন'; } alert("❌ Login Failed: " + error.message); });
}

function checkDeviceLimit(shopID, onSuccess) { return onSuccess(); }

function logoutSystem() {
    if(confirm("আপনি কি নিশ্চিতভাবে লগআউট করতে চান?")) {
        const showLoginScreen = () => {
            const keepAdmin = localStorage.getItem('smartpos_admin_pass'); const keepAppName = localStorage.getItem('smartpos_app_name'); const keepAppLogo = localStorage.getItem('smartpos_global_logo'); const keepDevId = localStorage.getItem('smartpos_device_id'); const keepTrial = localStorage.getItem('smartpos_trial_allowed');
            localStorage.clear();
            if(keepAdmin) localStorage.setItem('smartpos_admin_pass', keepAdmin); if(keepAppName) localStorage.setItem('smartpos_app_name', keepAppName); if(keepAppLogo) localStorage.setItem('smartpos_global_logo', keepAppLogo); if(keepDevId) localStorage.setItem('smartpos_device_id', keepDevId); if(keepTrial) localStorage.setItem('smartpos_trial_allowed', keepTrial);
            customersDB = []; productsDB = []; salesHistoryDB = []; dueCollectionHistoryDB = []; expensesDB = []; holdInvoices = []; cartItems = []; currentCustomerDue = 0; cartTotal = 0; shopName = "My Shop"; waNumber = ""; currentShopID = null;
            if(document.getElementById('prod-list-body')) document.getElementById('prod-list-body').innerHTML = ''; if(document.getElementById('cus-list-body')) document.getElementById('cus-list-body').innerHTML = ''; if(document.getElementById('sales-history-body')) document.getElementById('sales-history-body').innerHTML = ''; if(document.getElementById('due-list-body')) document.getElementById('due-list-body').innerHTML = ''; if(document.getElementById('exp-list-body')) document.getElementById('exp-list-body').innerHTML = ''; if(document.getElementById('cart-items')) document.getElementById('cart-items').innerHTML = ''; if(document.getElementById('dash-today-profit')) document.getElementById('dash-today-profit').innerText = '৳ 0';
            if(document.getElementById('display-profile-img')) document.getElementById('display-profile-img').src = "https://via.placeholder.com/100?text=User"; if(document.getElementById('profile-user-name')) document.getElementById('profile-user-name').value = ""; if(document.getElementById('profile-shop-name')) document.getElementById('profile-shop-name').value = ""; if(document.getElementById('profile-user-phone')) document.getElementById('profile-user-phone').value = ""; if(document.getElementById('profile-user-email')) document.getElementById('profile-user-email').value = ""; if(document.getElementById('profile-display-shopid')) document.getElementById('profile-display-shopid').value = "";
            const logoTextH2 = document.getElementById('main-logo-text'); if (logoTextH2) logoTextH2.innerHTML = `<i class="fa-solid fa-store"></i> <span id="shop-name-display">My Shop</span>`;
            switchPage('dashboard');
            let posCont = document.querySelector('.pos-container'); if(posCont) posCont.style.display = 'none'; let adminPanel = document.getElementById('super-admin-panel'); if(adminPanel) { adminPanel.style.display = 'none'; adminPanel.classList.add('hidden'); } let landing = document.getElementById('saas-landing'); if(landing) { landing.classList.remove('hidden'); landing.style.display = 'flex'; applyGlobalBranding(); }
            alert("✅ সফলভাবে লগআউট হয়েছে!");
        };
        if (firebase.auth().currentUser) { firebase.auth().signOut().then(() => { showLoginScreen(); }).catch(() => showLoginScreen()); } else { showLoginScreen(); }
    }
}